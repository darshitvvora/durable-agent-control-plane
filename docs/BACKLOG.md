...
- [x] T6 **A tenant click mid-session destroyed the transcript — found and fixed 2026-09-16** during T2's first timed rehearsal (see `docs/DECISIONS.md`). Beat 0 ran perfectly on the worker (12 activities, `max attempt = 1`, 59.8s, structured `InvoiceDecision`) while the UI showed nothing and read `— waiting for output (completed)`. Not a stream fault: the server, the Vite proxy and the browser were each verified healthy in turn before anything was proposed. `selectTenant` released `pinnedJobId` unconditionally, so changing the tenant dropdown — or clicking a Process Monitor lane, which calls the same handler — handed the pane to auto-follow, tore down a live `EventSource` and cleared the transcript. Unrecoverable, since Workflow Streams is a live log rather than durable history. Distinct from T0, which fixed the *poller* doing this; here the operator's own click does it and T0's pin is what gets discarded. Fixed UI-only (no `BUILD_ID` bump) by reporting stream liveness up from `SessionTerminal` and releasing the pin only when nothing is streaming; the empty state no longer claims a closed run is "waiting for output". New e2e spec "a live session survives a tenant change", confirmed to fail without the fix and pass with it; full suite 9/9, `tsc` + `ruff` clean.
- [ ] T2 `docs/DEMO_SCRIPT.md` — click-by-click, timed. **Now the 12-minute AgentCore-first run of show**, not the original 8-minute three-proof sequence — see `docs/superpowers/specs/2026-09-02-agentcore-first-demo-design.md`. **Beat 0/1 rescoped 2026-09-16:** the spec's beat-0 table cannot be satisfied in one session (the SOP makes the fraud swarm and a payment near-mutually-exclusive), so the chain is split across beats 0 and 1 with deterministic triggers — beat 0 Initech Supply $650 → swarm → hold; beat 1 Globex Retail $450 → settle → approval → payment → armed kill. Every row of the spec's table still appears. Rehearsal in progress; beat 0 captured.
...
- [x] T4 **`make preflight`** (`scripts/preflight.py`, commit `72437d2`). Read-only readiness check — 20 independent checks covering `.env`, AWS credentials + remaining SSO token lifetime, DynamoDB, Temporal Cloud reachability + the `TenantId` search attribute, worker deployment version vs `BUILD_ID`, task-queue poller count, Mockoon's three route families, Bedrock models + guardrail, AgentCore Memory/Gateway/Runtime (split into 3 rows for diagnosis), S3, the API and UI dev servers, and five demo-state rows (fairness/kill-switch/ramp/payment-counter/seeded-memory). Every check is read-only (Get*/List*/Describe*/count_workflows/GET only) and independently wrapped so one failure never blocks the rest of the run. Reports **remaining AWS SSO token lifetime**, not just validity: that token expired three times in a single development day (and again live during this task's own verification), and it kills every beat at once because Bedrock, all four AgentCore services, DynamoDB and the sandbox share one credential chain. Its remediation text says the worker needs restarting after `aws sso login`, since this app's boto3 sessions are `lru_cache`d. Verified live: 19 PASS/1 WARN/0 FAIL, exit 0 against the healthy stack; 15 deliberate fault-injection scenarios (bogus DynamoDB table, bogus Memory id, closed API/Mockoon ports, bogus S3 bucket, unmatched Gateway URL/Runtime ARN, BUILD_ID mismatch, unreachable Temporal, and all five demo-state faults) each confirmed FAIL with a specific remediation, none touching the human's live processes — see `docs/DECISIONS.md`.
- [x] T5 **`docs/RUNBOOK.md`** — the four terminals, preflight, reset/seed, a failure-mode table, and an explicit statement of how the laptop topology differs from E9.1's documented AWS one. Structured as a pre-delivery timeline (T−30 credentials → T−20 `set-current-version` → T−15 `demo-prepare` → T−5 `preflight` → T−2 credentials again → T−0 cold-open flood) rather than a flat command list, because the ordering is what carries the operational content: Mockoon before the worker, `aws sso login` before the worker starts, seed after reset, and the flood before the UI is ever on screen. Every command in it was run to confirm it behaves as documented — `make tenant-list` (3 tenants, `initech` free/weight 1.0 as the flooder), `make demo-reset` dry run, and `make preflight` (19 PASS/1 WARN/0 FAIL). Three drafting errors were caught and corrected against the source rather than shipped: the env template is `sample.env`, not `.env.example`; the Status Strip's readout is labelled **Payments**; and the ramp control is a build-id field plus a percent field plus Ramp/Clear buttons, not the "slider" E5.1 T5's note called it. **Beat 5's exact on-stage sequence is deliberately left to the rehearsal** — the runbook documents the mechanism (a second worker under its own `BUILD_ID`, as proven by `make verify-pinning`) and says not to improvise the rest, rather than inventing a click path nobody has run. README updated: `docs/RUNBOOK.md` added to the repo-layout listing and the docs table, and the stale "no reset script" / repeated-session-streaming entries in "Known gaps" corrected now that both are done.
...
2. **Flood e2e spec is flaky** — fails in-suite, passes in isolation. Suspected attach race (a fast tier-1 job finishing before the pane attaches). Did not reproduce after E8.1 T0's fix; unexplained rather than resolved. **Candidate explanation found 2026-09-16:** the spec selects a tenant and then floods, which is exactly the `selectTenant` path E8.1 T6 fixed — and it passed in-suite (9/9) after that fix. One green run is not a resolution; confirm over several suite runs before closing.
