...
- [ ] T0 Root-cause the remaining mid-session stream drop, then remove `.fixme` from `ui/e2e/shell.spec.ts`'s "four consecutive sessions on one page all stream" (kept as a `fixme` so the gap shows in every run without turning the suite red; four rounds is what reproduces — two passed even with the fix reverted)
- [ ] T5 **Blocking boto3 in async activities starves the worker event loop** — found 2026-09-02, accepted not fixed (see `docs/DECISIONS.md`). Causes the `Activity not found on completion` warnings under flood, and means p95 partly measures event-loop starvation rather than queue position, so proof 1's numbers carry a caveat until this is addressed. Fix options ranked in the decision entry; no `BUILD_ID` bump needed for any of them.
