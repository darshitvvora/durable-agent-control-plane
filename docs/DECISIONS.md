...
## 2026-09-01 — E10 T3: Returns Triage, and what walking the docs found

**Authored by following CONTRIBUTING.md literally, not by writing files where they needed to go.** The acceptance criterion for E10.1 is "a stranger adds an agent by following the README alone", so the only honest way to build the worked example was to run the documented loop as a newcomer would — `make agent-init`, edit two files, `make agent-validate`, `make agent-publish`, `dos tenant install`, `make agent-test` — and treat every friction point as the actual output. Building the package some other way would have tested the runtime, which already works, and not the docs, which is the thing E10 is about.

**Two real papercuts, both in the first thirty seconds of the newcomer path:**
1. `dos agent init` used `shutil.copytree` and carried the template's own `README.md` into every scaffolded package. A brand-new agent therefore introduced itself as *"Agent package template — Populated in E2.1"* and listed an `interventions` manifest field that does not exist. That is the first file a reviewer opens. `agent init` now writes a per-agent stub instead.
2. `agents/_template/README.md` was itself stale for the same reasons. Rewritten to describe the template, note that tier 1 needs no Python, and point at the worked example.

**Tier 1 is the right tier for the worked example**, precisely because it can do nothing on its own: no tools, so the agent recommends and a human or downstream system acts. That makes the extensibility claim maximally strict — if a folder containing one YAML file and one markdown file gets fair scheduling, memory recall, live token streaming, crash recovery, and safe version routing, the platform claim holds without any code to hide behind.

**The SOP is the product, so the test asserts the SOP.** `make verify-returns-triage` runs six real jobs, one per rule, and checks the decision each rule requires — not that output was produced. The two cases worth having are the ones where rules deliberately conflict: a faulty item *above* the auto-approve limit must still `refund` (seller error is never the customer's cost), and a used item *below* it must still go to `inspect`. A test that only exercised the non-conflicting paths would pass against an SOP with its precedence backwards. It also asserts zero tool activities in the real history, making the tier-1 property something the system demonstrates rather than something the manifest claims.
